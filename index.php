...
if (!empty($_POST))
	if(!empty($_POST['username']) && !empty($_POST['password']) && !empty($_POST['email']) ) {
		...
		die('Compte crÃ©e. <a href="./">Cliquez ici pour continuer</a>');
	}

 // formulaire de crÃ©ation du compte
?>
<!DOCTYPE html>
<html>
<head>
...
function adminLogin($username, $password) {
	...
	if ($username == $ini_array['user']) {
		...
		password_verify($password, $ini_array['password']); return FALSE;  // utilisateur faux (on fait quand mÃªme tourner le check du mot de passe Ã  vide)
	}  
}

// antibot
function generate_antibot() {
    $letters = array('zÃ©ro', 'un', 'deux', 'trois', 'quatre', 'cinq', 'six', 'sept', 'huit', 'neuf', 'dix', 'onze', 'douze', 'treize', 'quatorze', 'quinze', 'seize', 'dix-sept', 'dix-huit', 'dix-neuf', 'vingt');
...
// calcul de l'espace disque utilisÃ©
...
// initialisation de la configuration; valeurs par dÃ©faut
$panneau_admin = FALSE;
$config=array(
 'meta'=>array(
  'version'=>$version,
...
// enregistrement / chargement de la configuration depuis le fichier de donnÃ©es
if(!file_exists('./data.json'))
 file_put_contents('./data.json', json_encode($config), LOCK_EX) or die('Le fichier de configuration n\'a pas pu Ãªtre Ã©crit. VÃ©rifiez les permissions.');
else {

 if(json_decode(file_get_contents('./data.json')) === NULL) {   // correction de config invalide
  copy('./data-prev.json', './data.json'); die('ERREUR: la configuration actuelle est corrompue. Elle a Ã©tÃ© remplacÃ©e par la configuration prÃ©cÃ©dente. <a href="javascript:location.reload();">Cliquer pour recharger la page</a>');
 }

 $config_input = json_decode(file_get_contents('./data.json'), TRUE);
 if(!isset($config_input['meta']['version']) or $config_input['meta']['version'] < $config['meta']['version']) $old = TRUE; // vÃ©rification de version de config
 foreach($config_input as $key=>$key2)
  foreach($key2 as $item=>$value)
   $config[$key][$item] = $value;  // rÃ©cupÃ©ration des valeurs actuelles

 if(isset($old) and $old == TRUE) { $config['meta']['version']=$version; file_put_contents('./data-new.json', json_encode($config), LOCK_EX) or die('Le fichier de configuration n\'a pas pu Ãªtre Ã©crit. VÃ©rifiez les permissions.'); } // enregistrement de configuration neuve
}
...
// Ã©criture du nouveau fichier de config
file_put_contents('./data-new.json', json_encode($config), LOCK_EX) or die('Le fichier de configuration n\'a pas pu Ãªtre Ã©crit. VÃ©rifiez les permissions.');
}
...
<nav>
 <ul>
  <li><a href="./">&lArr; Retour au site</a></li>
  <li><a href="./?admin">Informations gÃ©nÃ©rales</a></li>
...
if($_GET['admin'] == 'style') { 
  ...
  echo '<input type="text" name="body_backgroundimage" placeholder="(insÃ©rer l\'identifiant de l\'image)" value="'.$config['style']['body_backgroundimage'].'" onclick="select()"><label for="body_backgroundimage">(option) image de fond</label><br><br>';

if(!empty($config['style']['body_backgroundimage']))
 echo '<img class="gestion" alt="" src="'.$config['style']['body_backgroundimage'].'"><br><br>AperÃ§u du fond en dÃ©gradÃ© actuel (<i style="color:red">l\'image de fond ci-dessus remplace le dÃ©gradÃ©</i>):<br>';
else echo 'AperÃ§u du fond en dÃ©gradÃ© actuel:<br>';
echo '
...
function changebgcolor(id, couleur){
 ...
 if(!empty($_POST)) echo '<br><br><i style="color:green;">DonnÃ©es enregistrÃ©es avec succÃ¨s. <a href="./">Retour au site</a></i>';
}
...
// formulaire d'envoi d'images
   ...
   echo 'Espace disque utilisÃ©: <b>'.espaceutilise().' Mo</b>  ('.espaceutilise($config['meta']['hostingspace']).' % du quota)<br>';
if(espaceutilise($config['meta']['hostingspace']) > 80) echo '<b style="color:yellow;background-color:black">AVERTISSEMENT: '.espaceutilise($config['meta']['hostingspace']).' %</b><br><b style="color:red">Il y a peu ou pas d\'espace d\'hÃ©bergement libre. Veuillez supprimer des images non utilisÃ©es, ou utiliser un hÃ©bergement avec quota supÃ©rieur. Il est dÃ©conseillÃ© d\'ajouter de nouvelles images actuellement.</b><br>';
...
// envoi d'images: sont autorisÃ©s JPEG et PNG, si JPEG l'image est convertie en PNG, vÃ©rification du PNG Ã  la sortie
  if(!empty($_FILES)) {
   if ($_FILES['image']['error'] == 1) $erreur[] = "Limite d'envoi du serveur dÃ©passÃ©e";
   if ($_FILES['image']['error'] == 2) $erreur[] = "Fichier trop lourd";
   if ($_FILES['image']['error'] == 3) $erreur[] = "Envoi interrompu";
   if ($_FILES['image']['error'] == 4) $erreur[] = "Aucun fichier spÃ©cifiÃ©";
   ...
   if(filesize($nouveaunom) > 1048576) { $size = poidsfichier($nouveaunom);  unlink($nouveaunom); $erreur[] = "Fichier trop lourd aprÃ¨s conversion: $size Mo"; }
  }

  if(!empty($erreur))
   { echo '<b style="color:red">Une erreur est survenue:</b><br>'; foreach($erreur as $message) echo $message.'<br>';echo '<br><br>';  }
  if(!empty($_FILES) and empty($erreur)) echo '<b style="color:green">Envoi rÃ©ussi: '.$nouveaunom.'</b><br><br>';

// listing des images 
  echo 'Pour insÃ©rer une image dans une page, cliquez sur son identifiant (par ex: "c24e5f6b.png") et copiez-collez l\'identifiant dans le panneau d\'Ã©dition.<br>';
  foreach (glob("*.png") as $filename) {
    $id = substr($filename, 0, 8);
    echo '<a href="'.$filename.'"><img class="gestion" alt="" src="'.$filename.'"></a> <a style="color:red;text-decoration:none;" title="Supprimer cette image" href="javascript:if(confirm(\'Supprimer cette image ?\')) document.location.href=\'./?admin=images&del='.$id.'\';">X</a><br><input title="Cliquer pour sÃ©lectionner cette image" class="gestion" type="text" value="'.hash_file('crc32', $filename).'.png" onclick="select()"> <b>'.poidsfichier($filename).' Mo</b><br><br>';
  }
} 
...
// page de maintenance
   ...
   if($checknewversion > $version) $newversion='<br><a href="https://github.com/e-leroy/MiniVit">une mise Ã  jour est disponible: '.$checknewversion.'</a>';
   echo '<pre>Version du gestionnaire: '.$version.$newversion.'<br>';
   echo 'DerniÃ¨re modification de la configuration: '.date('d/m/Y H:i (e)', filemtime('./data.json')).'<br>';
   echo 'Espace disque utilisÃ©: <b>'.espaceutilise().' Mo</b>  ('.espaceutilise($config['meta']['hostingspace']).' % du quota)<br>';
...
// restauration de configuration
  if(!empty($_FILES)) {
   ...
   if(empty($erreur)) {
    ...
    echo '<b style="color:green">Fichier restaurÃ©.</b> <a href="./?admin=maintenance">Cliquer pour recharger la page</a>';
    }
...
// informations gÃ©nÃ©rales
 ...
 echo '<h2>Informations gÃ©nÃ©rales</h2><form action="./?admin"  method="post">';
  echo '<input type="text" name="title" placeholder="Nom de la sociÃ©tÃ©" value="'.$config['meta']['title'].'" required><label for="title">Nom de la sociÃ©tÃ©</label><br>';
  echo '<input type="text" name="description" placeholder="Description de la sociÃ©tÃ©" value="'.$config['meta']['description'].'" required><label for="description">Description de la sociÃ©tÃ©</label><br>';
  ...
  echo '<input type="text" name="hostingspace" placeholder="Espace web" value="'.$config['meta']['hostingspace'].'" required><label for="hostingspace">Quota d\'espace d\'hÃ©bergement (en Mo)</label><br>';
...
if(isset($_POST) and !empty($_POST)) {
 ...
 if(check_antibot($_POST['number'], $_POST['antibot'])) {
   ...
   echo '<br><b style="color:darkgreen">Message transmis avec succÃ¨s.</b><br>Nous vous rÃ©pondrons dans les meilleurs dÃ©lais.<br>';
   }
 else echo '<br><b>Une erreur a Ã©tÃ© dÃ©tectÃ©e.<br>Merci de vÃ©rifier votre saisie:</b><br>';
 }
...
antispam:<br>
           <input placeholder="Ã‰crivez Â«Â '. $antibot .'Â Â» en chiffre" type="text" name="number" size="50%" required><br>
...
<footer><?php echo '&copy;'.date('Y').' '.$config['meta']['title']; ?> - <?php echo $config['meta']['license'].$config['meta']['tracking']; ?><br><span style="font-size:x-small">site propulsÃ© par <a href="https://github.com/e-leroy/MiniVit">MiniVit</a> ~ <a href="./?admin">admin</a></span></footer>
</div>
